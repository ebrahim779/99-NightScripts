...
--// Embedded Library (Late's UI Library with Key System)
local Library = {}
...
local Setup = {
	...
	CorrectKey = "al50", -- Key system integrated here
}
...
local SetProperty = function(Object, Properties)
	for Index, Property in next, Properties do
		pcall(function() Object[Index] = Property end) -- Error handling
	end
	return Object
end
...
local Color = function(Color, Factor, Mode)
	...
	return Mode == "Light" and Color3.fromRGB((Color.R * 255) - Factor, (Color.G * 255) - Factor, (Color.B * 255) - Factor) or
		Color3.fromRGB((Color.R * 255) + Factor, (Color.G * 255) + Factor, (Color.B * 255) + Factor)
end
...
local Drag = function(Canvas)
	if Canvas then
		...
		local function Update(input)
			...
			pcall(function() Canvas.Position = UDim2.new(StartPosition.X.Scale, StartPosition.X.Offset + delta.X, StartPosition.Y.Scale, StartPosition.Y.Offset + delta.Y) end)
		end
...
KeyTitle.TextColor3 = Theme.Title
...
KeyMessage.TextColor3 = Theme.Description
...
KeyInput.FocusLost:Connect(function(enterPressed)
	if enterPressed then
		if KeyInput.Text == Setup.CorrectKey then
			KeyMessage.Text = "Access Granted!"
			task.wait(1)
			KeyFrame:Destroy()
			MainFrame.Visible = true
			Animations:Open(MainFrame, Setup.Transparency)
		else
...
function Animations:Open(Window, Transparency, UseCurrentSize)
	...
	pcall(function()
		...
		if Shadow then Tween(Shadow, 0.25, { Transparency = 0.5 }) end
		Tween(Window, 0.25, {
			Size = Original,
...
function Animations:Close(Window)
	...
	pcall(function()
		...
		if Shadow then Tween(Shadow, 0.25, { Transparency = 1 }) end
		Tween(Window, 0.25, {
			Size = Multiplied,
			GroupTransparency = 1,
		})
...
function Library:CreateWindow(Settings)
	...
	function Options:AddDropdown(Settings)
		...
		Connect(Dropdown.MouseButton1Click, function()
			...
			for Index, Option in next, Settings.Options do
				...
				SetProperty(Button, { Parent = Example, Visible = true })
				Destroy(Description)
				Connect(Button.MouseButton1Click, function()
	...
	local Themes = {
		Names = {
			...
			["TextLabel"] = function(Label) if Label:FindFirstChild("Padding") then Label.TextColor3 = Theme.Tab end end,
	...
	SetProperty(Window, { Size = Settings.Size, Visible = false, Parent = Screen })
...
local function createCommandGui()
	...
	local commandMessage = Instance.new("TextLabel")

	commandFrame.Parent = Screen
	commandFrame.Size = UDim2.new(0, 300, 0, 50)
	commandFrame.Position = UDim2.new(0.5, -150, 0, 10)
	commandFrame.BackgroundColor3 = Theme.Primary
	commandFrame.BorderSizePixel = 0
	commandFrame.Visible = false

	commandInput.Size = UDim2.new(0.9, 0, 0.6, 0)
	commandInput.Position = UDim2.new(0.05, 0, 0.2, 0)
	commandInput.BackgroundColor3 = Theme.Component
	commandInput.TextColor3 = Theme.Title
	commandInput.PlaceholderText = "Enter command (/esp, /killaura, /speed, /autofarm)"
	commandInput.TextSize = 14
	commandInput.Font = Enum.Font.Gotham
	commandInput.Parent = commandFrame

	commandMessage.Size = UDim2.new(1, 0, 0, 20)
	commandMessage.Position = UDim2.new(0, 0, 1, 5)
	commandMessage.BackgroundTransparency = 1
	commandMessage.TextColor3 = Theme.Description
	commandMessage.TextSize = 12
	commandMessage.Text = ""
	commandMessage.Parent = commandFrame
	...
	commandInput.FocusLost:Connect(function(enterPressed)
		if enterPressed then
			local command = commandInput.Text:lower():sub(2)
			if command == "esp" then
				commands.esp = not commands.esp
				commandMessage.Text = "ESP " .. (commands.esp and "enabled" or "disabled")
				if commands.esp then enableESP() else disableESP() end
			elseif command == "killaura" then
				commands.killaura = not commands.killaura
				commandMessage.Text = "Kill Aura " .. (commands.killaura and "enabled" or "disabled")
			elseif command == "speed" then
				commands.speed = not commands.speed
				commandMessage.Text = "Speed Hack " .. (commands.speed and "enabled" or "disabled")
				if Character and Humanoid then
					Humanoid.WalkSpeed = commands.speed and 50 or 16
				end
			elseif command == "autofarm" then
				commands.autofarm = not commands.autofarm
				commandMessage.Text = "Auto Farm " .. (commands.autofarm and "enabled" or "disabled")
			else
				commandMessage.Text = "Unknown command!"
			end
			task.wait(2)
			commandMessage.Text = ""
			commandInput.Text = ""
	...
	local function updateESP()
		for _, obj in pairs(Workspace:GetChildren()) do
			if obj:IsA("Model") and obj:FindFirstChild("HumanoidRootPart") then
				...
				if name:find("bear") or name:find("wolf") or name:find("animal") then
					if not obj.HumanoidRootPart:FindFirstChild("ESP") then
		...
		if Character and RootPart then
			for _, obj in pairs(Workspace:GetChildren()) do
				if obj:IsA("Model") and obj:FindFirstChild("Humanoid") and obj:FindFirstChild("HumanoidRootPart") then
					...
					if distance <= 15 then
						pcall(function() obj.Humanoid.Health = 0 end)
					end
				end
			end
		end
...
local function enableAutoFarm()
	...
	local function autoFarm()
		if Character and RootPart then
			for _, obj in pairs(Workspace:GetDescendants()) do
				if obj:IsA("Part") and (obj.Name:lower():find("wood") or obj.Name:lower():find("stone") or obj.Name:lower():find("berry")) then
					pcall(function() obj.CFrame = RootPart.CFrame + Vector3.new(0, 2, 0) end)
				end
			end
		end
	end
...
local function createMainWindow()
	...
	Window:AddParagraph({ Title = "Welcome", Description = "Welcome to 99 Nights Script! Use buttons or commands (/esp, /killaura, /speed, /autofarm).", Tab = Main })
	...
	Window:AddToggle({
		...
		Callback = function(Boolean)
			...
			commands.killaura = Boolean
			if KillAuraEnabled then enableKillAura() end
		end,
	})
	...
	Window:AddToggle({
		...
		Callback = function(Boolean)
			...
			commands.autofarm = Boolean
			if AutoFarmEnabled then enableAutoFarm() end
		end,
	})
	...
	Window:AddButton({
		...
		Tab = Main,
		Callback = function()
			Window:Notify({ Title = "Test", Description = "This is a test notification", Duration = 5 })
		end,
	})
	...
	Window:AddToggle({
		Title = "UI Blur",
		Description = "Enable UI blur (requires graphics 8+)",
		Default = true,
		Tab = Settings,
		Callback = function(Boolean)
	...
	Window:Notify({ Title = "Hello!", Description = "Press Left Alt to toggle UI. Use /commands.", Duration = 10 })
...
local function handleErrors()
	while wait(1) do
		...
		if not success then
			...
			-- Attempt to reload key frame if main window fails
